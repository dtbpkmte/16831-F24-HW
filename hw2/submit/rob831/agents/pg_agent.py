...
class PGAgent(BaseAgent):
    def __init__(self, env, agent_params):
        ...
        self.actor = MLPPolicyPG(
            ...
            n_grad_steps=self.agent_params['n_grad_steps']
        )
    ...
    def _discounted_return(self, rewards):
        ...
        discounted_return = np.sum(np.array(rewards) * (self.gamma ** np.arange(len(rewards))))
        discounted_returns = np.array([discounted_return] * len(rewards))
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for t in reversed(range(len(rewards) - 1)):
            discounted_cumsums[t] = discounted_cumsums[t + 1]*self.gamma + rewards[t]
