...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        if self.params['n_envs'] == 1:
            ...
            wrapper = None if params['action_noise_std'] == 0 else lambda e: ActionNoiseWrapper(e, seed, params['action_noise_std'])
            self.env = gym.vector.make(self.params['env_name'],
                                       self.params['n_envs'],
                                       asynchronous=True,
                                       wrappers=wrapper)
        ...
        obs_space = self.env.observation_space if not isinstance(self.env, gym.vector.AsyncVectorEnv) \
            else self.env.single_observation_space
        ac_space = self.env.action_space if not isinstance(self.env, gym.vector.AsyncVectorEnv) \
            else self.env.single_action_space
        ob_dim = obs_space.shape if img else obs_space.shape[0]
        ac_dim = ac_space.n if discrete else ac_space.shape[0]
        self.params['agent_params']['ac_dim'] = ac_dim
        self.params['agent_params']['ob_dim'] = ob_dim

        metadata_keys = gym.make(self.params['env_name']).metadata.keys() \
            if isinstance(self.env, gym.vector.AsyncVectorEnv) \
            else self.env.env.metadata.keys()
        ...
        elif 'video.frames_per_second' in metadata_keys:
